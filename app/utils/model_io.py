...
def save_model(
    ...
    """Write name_timestamp.pt and a matching json into save_dir. Returns the two paths."""
    try:
...
def load_model_metadata(json_path: Union[str, Path]) -> Dict[str, Any]:
    """Read the json that sits next to a saved model."""
...
def list_saved_models(save_dir: Union[str, Path]) -> List[Dict[str, Any]]:
    """List saved models in save_dir, newest first. Missing folders come back empty."""
    save_dir = Path(save_dir)
...
def format_created_at(
    ...
    """Turn a saved ISO timestamp into something readable, or empty if it is missing or junk."""
    if not created_at:
        return ""
    try:
...
def mean_r2(scores: Dict[str, float]) -> float:
    """Average Water, Solids, and Bitumen R². Same number we use to pick the best checkpoint."""
...
def model_r2_splits(
    ...
    """Pull validation and test R² out of metadata. Val comes from best_val_r2, or from the best-mean epoch in training_history on older files."""
...
def resolve_model_r2(metadata: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Headline accuracy: test R² if we saved it, otherwise validation."""
...
def format_r2_headline(metadata: Optional[Dict[str, Any]]) -> str:
    """Short label like "R² 0.42" from the headline split, or empty if none was recorded."""
