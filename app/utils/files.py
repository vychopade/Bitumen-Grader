...
def collect_from_urls(
    ...
    """Turn dropped URLs into local file paths. Walks folders when recurse_dirs is set."""
...
def urls_have_accepted_files(
    ...
    """True if the drop has at least one matching file, or a folder when recurse_dirs is on."""
    suffixes = tuple(ext.lower() for ext in extensions)
    for url in urls:
        if not url.isLocalFile():
            continue
...
def dropped_local_paths(
    ...
    *,
    ...
    """Matching local files from a drop event. Set recurse_dirs to search dropped folders."""
    mime = event.mimeData()
