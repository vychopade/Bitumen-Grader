...
class _Sidebar(QWidget):
    ...
    def set_active_index(self, index: int) -> None:
        """Check the sidebar button for this page so it looks selected."""
    ...
    def set_active_model_label(
        ...
        """Show the loaded model name at the bottom of the sidebar, or No model if nothing is loaded. The optional R² headline goes on the tooltip."""
...
class MainWindow(QMainWindow):
    ...
    def navigate_to_index(self, index: int) -> None:
        """Switch to the sidebar page at this index and light up that nav button."""
    ...
    def navigate_to(self, page_key: str) -> None:
        """Open a page by name: train, grade, or models."""
    ...
    def page_for(self, page_key: str) -> Optional[QWidget]:
        """Look up a stacked page by name (train, grade, or models)."""
        keys = [item[0] for item in _NAV_ITEMS]
        try:
    ...
    def set_active_model(
        ...
        """Load a checkpoint into a predictor and tell the sidebar and pages. Pass None to clear. If the file will not load we show a dialog and leave the previous model alone."""
        ...
        try:
            ...
            name = metadata.get("name") or Path(model_path).stem
            alert(
                self,
                title="Model Load Failed",
                message=f"Couldn\u2019t load \u201c{name}\u201d.",
