...
class RegressionPredictor:
    ...
    def __init__(self, model_path, metadata):
        ...
        self._mean = torch.tensor(
            [float(self.output_stats[name]["mean"]) for name in self.output_names],
            dtype=torch.float32,
        )
        self._std = torch.tensor(
            [float(self.output_stats[name]["std"]) for name in self.output_names],
            dtype=torch.float32,
        )
    ...
    def _prepare_source(self, source: ImageSource) -> Image.Image:
        return prepare_image(source, self.image_size, square=not self._legacy_crop)
    ...
    def _decode_chunk(self, sources: Sequence[ImageSource]) -> List[Optional[torch.Tensor]]:
        ...
        if len(sources) <= 1 or self._decode_workers <= 1:
            ...
            with ThreadPoolExecutor(max_workers=workers) as pool:
                futures = [pool.submit(self._prepare_source, source) for source in sources]
                prepared = []
    ...
    def predict_many(
        ...
        *,
        ...
        """Grade many photos with batched inference. Failed items are ``None``."""
        total = len(sources)
        ...
        for start in range(0, total, chunk_size):
            ...
            ready_index = [index for index, tensor in enumerate(decoded) if tensor is not None]
            if ready_index:
                ...
                for local_index, result in zip(ready_index, self._dicts_from_raw(raw_outputs)):
