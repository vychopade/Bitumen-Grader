...
class RegressionPredictor:
    """Loads a trained checkpoint and predicts water, solids, and bitumen percent. The two strongest heads are predicted; the third is 100 minus those two so the grades always add to 100."""
    ...
    def __init__(self, model_path, metadata):
        ...
        if is_triple_payload(payload):
            self.model = TripleBitumenRegressor.from_payload(
                payload, self.metadata, device
            )
        else:
        ...
        self.output_stats = self.metadata.get("output_stats") or {}
        missing_stats = [
            name for name in OUTPUT_NAMES if name not in self.output_stats
        ]
        if missing_stats:
            raise ValueError(
                "This model file is missing training statistics for "
                f"{', '.join(missing_stats)}, so it cannot be used for grading."
            )
        # Old checkpoints may have used raw percents. New runs z-score labels.
        self.normalise_targets = bool(
            self.metadata.get("normalise_targets", True)
        )
        self.output_names = list(OUTPUT_NAMES)
        self.residual_output = self.metadata.get("residual_output")
        if self.residual_output not in self.output_names:
            self.residual_output = choose_residual_output(
                self.metadata.get("head_val_r2")
                or self.metadata.get("best_val_r2"),
                self.metadata.get("head_val_mae")
                or self.metadata.get("best_val_mae"),
            )
        self.predicted_names = list(
            self.metadata.get("predicted_outputs")
            or predicted_outputs(self.residual_output)
        )
        self.model.eval()
        ...
        self._preserve_aspect = bool(
            self.metadata.get("preserve_aspect_ratio", False)
        )
        self.transform = build_eval_transforms(
            image_size,
            legacy_crop=self._legacy_crop,
    ...
    def _dicts_from_raw(self, raw_batch: torch.Tensor) -> List[dict]:
        ...
        values = close_composition(values, self.residual_output)
        results = []
            ...
            results.append(
                {
                    ...
                    "predicted_outputs": list(self.predicted_names),
                }
            )
        return results
    ...
    def predict_many(
        ...
        *,
        ...
        """Grade many photos in batches. Failed items come back as None so one bad file does not stop the rest."""
        total = len(sources)
