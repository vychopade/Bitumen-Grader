...
from app.ml.recipe import CLS_BINS, IMAGE_SIZE, TEST_FRACTION, VAL_FRACTION
...
def parse_campaign_id(filename: str, row=None) -> str:
    """Which flotation campaign a photo belongs to, so we can hold out whole runs. Returns unknown if nothing matched."""
    if row is not None:
...
class RegressionDataset(Dataset):
    """Match labels-file rows to photos, then keep one split. Means and stds always come from the train split even if this instance is val or test. split_mode random shuffles photos; experiment holds out whole flotation campaigns."""
    ...
    def __init__(
        ...
        self.val_transforms = build_eval_transforms(
            self.image_size,
            legacy_crop=self.legacy_crop,
            preserve_aspect=not self.legacy_crop,
        )
        self.transforms = (
    ...
    def _index_images(image_dir: Path):
        """Index every photo under image_dir by basename, relative path, and stem so a CSV name can find it."""
        by_name = {}
        by_stem = {}
    ...
    def _split_by_campaign(self, matched, val_fraction, test_fraction, seed):
        """Put whole flotation campaigns into train, val, or test instead of mixing photos from the same run. Returns None if we only found one campaign."""
    ...
    def _split_portions(shuffled, val_fraction, test_fraction):
        """Cut a shuffled list into train, val, and test. Train is never left empty if we can help it."""
        total = len(shuffled)
        if total == 0:
            return [], [], []
    ...
    def _parse_float(raw_value, column_name):
        """Turn a table cell into a float, or raise a ValueError that names the column if it was blank or junk."""
        try:
    ...
    def _parse_pan(raw_value):
        """Turn the Pan cell into a whole number. Strings like 3.0 are fine; blanks are not."""
        try:
    ...
    def _equal_frequency_edges(values, n_bins=CLS_BINS):
        """Cut the train labels into equal-count bins and return the interior edges, or empty if there is not enough data."""
