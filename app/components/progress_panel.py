...
class ProgressPanel(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self._epochs_per_head = 0
        self._patience = 0
        self._best_val_loss = float("inf")
        self._patience_counter = 0
        self._active_head: Optional[str] = None
        self._head_epoch = 0
    ...
    def reset(self, total_epochs: int, patience: int) -> None:
        """Clear the charts and log and set the epoch bar for a new run."""
        self._total_epochs = total_epochs
        self._epochs_per_head = max(1, total_epochs // 3)
        ...
        self._epoch_label.setText(
            f"Water  \u00b7  Epoch 0 / {self._epochs_per_head}"
        )
        self._patience_label.setVisible(False)
        ...
        self.append_log(
            f"Starting training \u2014 Water, Solids, then Bitumen "
            f"({self._epochs_per_head} epochs each)\u2026"
        )
    ...
    def update_progress(
        ...
        """Redraw the metrics, charts, and log after one epoch."""
        r2 = val_r2_dict or {}
        present = [name for name in OUTPUT_NAMES if name in r2]
        single_head = len(present) == 1
        active_head = present[0] if single_head else None

        if active_head and active_head != self._active_head:
            self._active_head = active_head
            self._head_epoch = 0
            self.append_log(f"Training {active_head}\u2026")
        if active_head:
            self._head_epoch += 1
            self._epoch_label.setText(
                f"{active_head}  \u00b7  Epoch {self._head_epoch} / "
                f"{self._epochs_per_head}"
            )
        else:
            self._epoch_label.setText(
        ...
        water_r2 = r2.get("Water", float("nan"))
        solids_r2 = r2.get("Solids", float("nan"))
        bitumen_r2 = r2.get("Bitumen", float("nan"))
        ...
        self._water_r2s.append(water_r2 if "Water" in r2 else float("nan"))
        self._solids_r2s.append(solids_r2 if "Solids" in r2 else float("nan"))
        self._bitumen_r2s.append(
            bitumen_r2 if "Bitumen" in r2 else float("nan")
        )
        ...
        if single_head:
            name = active_head
            mae = val_mae_dict.get(name, 0.0)
            score = r2.get(name, 0.0)
            self.append_log(
                f"{name} epoch {self._head_epoch}/{self._epochs_per_head} "
                f"\u2014 Loss: {train_loss:.4f} | Val: {val_loss:.4f} "
                f"| R² {score:.3f} | MAE \u00b1{mae:.2f}%"
            )
        else:
            self.append_log(
                f"Epoch {epoch}/{self._total_epochs} \u2014 Loss: "
                f"{train_loss:.4f} | Val: {val_loss:.4f} "
                f"| R² Bitumen: {bitumen_r2:.3f}  Solids: {solids_r2:.3f}  "
                f"Water: {water_r2:.3f} "
    ...
    def show_early_stopped_banner(
        ...
        """Show the amber early-stop banner instead of the green one."""
        self._completion_banner.setVisible(False)
        text = (
