...
from app.components.dialogs import confirm_destructive
...
class ModelCard(QFrame):
    ...
    def _build_pills_row(self) -> QHBoxLayout:
        ...
        residual = self.metadata.get("residual_output")
        predicted = self.metadata.get("predicted_outputs") or []
        if residual and len(predicted) == 2:
            bits.append(
                f"{predicted[0]} & {predicted[1]} predicted · "
                f"{residual} by difference"
            )
        elif self.metadata.get("composition") == "two_plus_residual":
            bits.append("grades close to 100%")
        epoch_count = self.metadata.get("final_epoch") or len(
            self.metadata.get("training_history") or []
        )
        if epoch_count:
            bits.append(
                f"{epoch_count} epoch{'s' if epoch_count != 1 else ''}"
            )
    ...
    def _build_action_row(self) -> QHBoxLayout:
        ...
        self._load_button.clicked.connect(
            lambda: self.load_requested.emit(self.metadata)
        )
        ...
        self._retrain_button.setToolTip(
            "Continue this model on a new labelled dataset"
        )
        self._retrain_button.setStyleSheet(ghost_button_qss())
        self._retrain_button.clicked.connect(
            lambda: self.retrain_requested.emit(self.metadata)
        )
    ...
    def _build_details_section(self) -> QWidget:
        ...
        separator.setStyleSheet(
            f"background-color: {BORDER_COLOR}; border: none;"
        )
    ...
    def _build_no_history_placeholder() -> QWidget:
        ...
        label.setStyleSheet(
            ...
            f" background-color: {BACKGROUND_COLOR}; border-radius: 6px;"
        )
        return label
    ...
    def _build_r2_table(self) -> Optional[QTableWidget]:
        ...
        for row, label in enumerate(OUTPUT_NAMES):
            values = [label] + [
                f"{column.get(label, 0.0):.2f}" for column in columns
            ]
    ...
    def set_active(self, is_active: bool) -> None:
        """Show or hide the in-use badge and accent border when this is the loaded model."""
        self._is_active = is_active
    ...
    def _confirm_delete(self) -> None:
        ...
        confirmed = confirm_destructive(
            ...
            detail=(
                "This permanently removes the trained weights and the "
                "metadata file saved next to them."
            ),
        )
        if confirmed:
