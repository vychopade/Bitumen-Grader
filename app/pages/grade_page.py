"""Grade page. You drop photos onto the queue, run the loaded model, and see water, solids, and bitumen. The three percentages always add to 100. There is also a rough guess of which pan batch the bitumen looks like."""
...
class GradePage(QWidget):
    ...
    def _build_batch_table(self) -> QTableWidget:
        ...
        table.setHorizontalHeaderLabels(
            ["#", "Filename", "Water", "Solids", "Bitumen", "Batch"]
        )
        table.verticalHeader().setVisible(False)
        table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)
    ...
    def _handle_model_load_failure(self, exc: Exception) -> None:
        ...
        alert(
            ...
            message="That model couldn\u2019t be loaded, so it\u2019s been cleared.",
            detail=message,
        )
    ...
    def _update_measurement_table(
        ...
        for row, label in enumerate(OUTPUT_NAMES):
            ...
            training_avg = output_stats.get(label, {}).get("mean", 0.0)
            pred_item = QTableWidgetItem(f"{predicted_value:.2f}")
            avg_item = QTableWidgetItem(f"{training_avg:.2f}")
    ...
    def _refresh_batch_table(self) -> None:
        ...
        for row, queue_image in enumerate(graded):
            ...
            table.setItem(row, 5, grade_item)
    ...
    def _on_export_results(self) -> None:
        ...
        header = ["filename", "water", "solids", "bitumen", "batch"]
        rows: List[List[str]] = []
