...
def _approx_output_range(mean: float, std: float) -> Tuple[float, float]:
    """A rough min and max from the training mean plus or minus two standard deviations, clamped to 0–100."""
...
def _closest_pan_grade(
    ...
    """Guess which pan batch (3 through 6) a predicted bitumen percent looks like. We do not have a saved mapping, so we split the training bitumen range into four equal bins."""
