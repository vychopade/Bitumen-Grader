...
from app.components.image_editor import ImageEditor
from app.constants import IMAGE_EXTENSIONS
from app.pages.import_widgets import (
    THUMBNAIL_SIZE,
    _DropZone,
    _LoadedImage,
    _Thumbnail,
...
from app.paths import edited_images_dir
...
class ImportPage(QWidget):
    ...
    def _add_images(self, paths: List[str]) -> None:
        ...
        new_paths: List[str] = []
        for path in paths:
            if not path.lower().endswith(IMAGE_EXTENSIONS) or path in already_known:
                continue
