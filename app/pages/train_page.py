...
class TrainPage(QWidget):
    ...
    def _build_csv_section(self) -> QFrame:
        ...
        mapping_form.setFieldGrowthPolicy(
            QFormLayout.FieldGrowthPolicy.AllNonFixedFieldsGrow
        )
        mapping_form.setLabelAlignment(
            Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter
        )
        self._add_mapping_row(mapping_form, "Filename column:", "Image")
        self._add_outputs_row(mapping_form)
    ...
    def _mapping_label(self, text: str) -> QLabel:
        label = QLabel(text)
        label.setStyleSheet(
            f"color: {TEXT_SECONDARY}; font-size: 11px;"
            f" background: transparent;"
    ...
    def _add_mapping_row(
        ...
        form.addRow(self._mapping_label(label_text), value)

    def _add_outputs_row(self, form: QFormLayout) -> None:
        names = QWidget()
        names.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred
        )
        ...
        for name in OUTPUT_NAMES:
            ...
            row.addWidget(chip)
        row.addStretch(1)
        form.addRow(self._mapping_label("Outputs to predict:"), names)
    ...
    def _refresh_continue_combo(self) -> None:
        if self._continue_combo is None:
            return
        previous_data = self._continue_combo.currentData()
        previous_path = (
            previous_data.get("model_path")
    ...
    def prepare_retrain(self, metadata: Dict) -> None:
        """Tick Continue training and select this saved model so the user can run it on a new labels file."""
    ...
    def _apply_image_folder(self, folder: str) -> bool:
        """Set the photo folder for step 2. Returns False if we could not read it."""
        try:
    ...
    def _on_start_training(self) -> None:
        ...
        try:
            if continuing:
                ...
                models = self._heads_from_saved_model(parent_meta)
            else:
                models = {
                    name: BitumenRegressor(
    ...
    @staticmethod
    def _heads_from_saved_model(
        parent_meta: Dict,
    ) -> Dict[str, BitumenRegressor]:
        """Reload the three saved heads so Continue training actually starts from those weights."""
        try:
            return TripleBitumenRegressor.heads_from_checkpoint(
                parent_meta.get("model_path"), parent_meta, device="cpu"
            )
        except ValueError as exc:
            if "triple-head" not in str(exc):
                raise
            raise ValueError(
                "This saved model is an older single-network file. "
                "Train a new model instead of continuing it."
            ) from exc
    ...
    def _launch_training_thread(self, trainer: RegressionTrainer) -> None:
        ...
        self._progress_panel.reset(trainer.num_epochs * 3, trainer.patience)
    ...
    def _on_training_finished(self, result: TripleRegressionResult) -> None:
        parts = {
            "Water": result.water,
            "Solids": result.solids,
            "Bitumen": result.bitumen,
        }
        best_val_mae = result.val_composition_mae or {
            name: (part.best_val_mae or {}).get(name, 0.0)
            for name, part in parts.items()
        }
        best_val_r2 = result.val_composition_r2 or {
            name: (part.best_val_r2 or {}).get(name, 0.0)
            for name, part in parts.items()
        }
        test_mae = result.test_normalised_mae
        test_r2 = result.test_r2 or {
            name: (part.test_r2 or {}).get(name, 0.0)
            for name, part in parts.items()
        }
        stopped_early = any(part.stopped_early for part in parts.values())
        final_epoch = max(part.final_epoch for part in parts.values())
        residual = result.residual_output or "Solids"
        predicted = result.predicted_outputs or [
            name for name in OUTPUT_NAMES if name != residual
        ]

        try:
            extra = dict(self._pending_train_config)
            extra["residual_output"] = residual
            extra["predicted_outputs"] = list(predicted)
            extra["composition"] = "two_plus_residual"
            extra["num_outputs"] = 3
            extra["head_val_r2"] = {
                name: (part.best_val_r2 or {}).get(name, 0.0)
                for name, part in parts.items()
            ...
            extra["test_normalised_sum_deviation"] = (
            ...
            paths = save_model(
                ...
                output_stats=merged.output_stats,
                result=merged,
                save_dir=MODELS_DIR,
        ...
        predicted_label = " and ".join(predicted)
        self._progress_panel.append_log(
            f"Predicts {predicted_label}; {residual} is 100% minus those two."
        )
